defmodule ProjectNameWeb.Helpers.GetSvgicons do
	alias PortfolioWeb.Router.Helpers, as: Routes

	# Synopsis:
	# raw get_svgicon("bi bi-caret-right-fill")
	def get_svgicon(classname) do
		"#{get_svgcode(:unique, classname)}"
	end

	# Synopsis: 
	#	raw get_svgallicons()
	def get_svgallicons() do
		"#{get_svgcode(:all, nil)}"
	end


	defp get_svgcode(mode, classname) do

bootstrapsicons = %{"bi bi-caret-right-fill" => 
~S"""
...
"""
		...
		case mode do
			:unique ->
				"#{bootstrapsicons[classname]}"
			:all ->
					for {k, v} <- bootstrapsicons  do
							"<div class=\"card card-floated\"><div class=\"card-body\"><h1 class=\"card-title\">#{v}</h1><p class=\"card-text\">#{k}</p></div></div>"
					end
			_	->
				"nothing"
		end
